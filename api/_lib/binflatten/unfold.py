...
def unfold(model: Model, params: FlattenParams) -> FlatPattern:
    ...
    while remaining:
        ...
        if not options:
            ...
            break
        # Quantise the overlap before ranking: every collision-free candidate
        # sits within float noise of zero, and letting sub-µm² noise pick the
        # winner makes the chosen hinge depend on the CPU / library build
        # (observed: the half bins' front wall hinged on a different side
        # wall on Vercel's x86 Linux than on the arm64 Mac that froze the
        # goldens). 1e-3 mm² is far below any real collision (the toes are
        # ~118 mm²); ties then break by child/parent face id so the same net
        # comes out on every machine.
        options.sort(key=lambda o: (round(o[0], 3), o[1], o[2]))
