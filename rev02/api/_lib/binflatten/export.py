...
def _canon_ring(coords):
    """Deterministic ring serialisation. GEOS builds differ in which vertex a
    union/buffer ring 'starts' at and in its winding — the cut is identical
    but the file isn't reproducible across machines. Normalise: drop the
    closing duplicate, wind counter-clockwise, start at the lexicographically
    smallest vertex, re-close."""
    pts = np.asarray(coords, float)
    if len(pts) > 1 and np.allclose(pts[0], pts[-1]):
        pts = pts[:-1]
    x, y = pts[:, 0], pts[:, 1]
    if np.sum(x * np.roll(y, -1) - np.roll(x, -1) * y) < 0:   # shoelace
        pts = pts[::-1]
    i = np.lexsort((pts[:, 1], pts[:, 0]))[0]
...
def build_geometry(fp: FlatPattern, params: FlattenParams) -> LaserGeometry:
    ...
    # Drop boolean-op debris: a union/buffer can leave a vertex that is
    # collinear with its neighbours to within float noise, and GEOS builds
    # differ in whether it appears at all — same cut, different file. 0.1 µm
    # is 1000x below the kerf, so this never moves real geometry.
    union = union.simplify(1e-4, preserve_topology=True)

    pieces = list(union.geoms) if isinstance(union, MultiPolygon) else [union]
    # deterministic order/serialisation (see _canon_ring): pieces and holes by
    # position, every ring CCW from its smallest vertex
    pieces.sort(key=lambda p: (round(p.bounds[0], 6), round(p.bounds[1], 6)))
    warnings = []
    dropped = 0
    cut_loops = []
    for piece in pieces:
        ...
        for ring in piece.interiors:
            if Polygon(ring).area < params.min_hole_area_mm2:
                dropped += 1
                continue
            holes.append(_canon_ring(ring.coords))
        holes.sort(key=lambda r: (round(r[:, 0].min(), 6), round(r[:, 1].min(), 6)))
        cut_loops.append([_canon_ring(piece.exterior.coords)] + holes)
    if dropped:
        warnings.append(
